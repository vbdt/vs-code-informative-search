...
import { InformativeSearchTreeProvider } from './views/treeViewProvider';
import { SearchConfiguration, DEFAULT_SEARCH_CONFIG, CategorizedResults } from './search/searchTypes';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const treeProvider = new InformativeSearchTreeProvider();
    ...
    context.subscriptions.push(
        ...
        openSearchSettingsCommand,
        openFileCommand,
        openFileToSideCommand,
        copyPathCommand,
        copyLineContentCommand,
        treeView
    );
    ...
    if (isFirstActivation) {
        vscode.window.showInformationMessage(
            'Informative Search is ready! Use Ctrl+Shift+F to start searching.',
 ...
 */
async function performCurrentFileSearch(
    searchProvider: SearchProvider, 
    treeProvider: InformativeSearchTreeProvider,
    searchTerm?: string
) {
...
function getSearchConfiguration(): SearchConfiguration {
    ...
    return {
        includePatterns: config.get('includePatterns', DEFAULT_SEARCH_CONFIG.includePatterns),
        excludePatterns: config.get('excludePatterns', DEFAULT_SEARCH_CONFIG.excludePatterns),
        includeComments: config.get('includeComments', DEFAULT_SEARCH_CONFIG.includeComments),
        includeStringLiterals: config.get('includeStringLiterals', DEFAULT_SEARCH_CONFIG.includeStringLiterals),
        caseSensitive: config.get('caseSensitive', DEFAULT_SEARCH_CONFIG.caseSensitive),
        useRegex: config.get('useRegex', DEFAULT_SEARCH_CONFIG.useRegex),
        maxResultsPerCategory: config.get('maxResultsPerCategory', DEFAULT_SEARCH_CONFIG.maxResultsPerCategory)
    };
}
 ...
 */
async function exportSearchResults(results: CategorizedResults) {
    const saveUri = await vscode.window.showSaveDialog({
        defaultUri: vscode.Uri.joinPath(
            vscode.workspace.workspaceFolders?.[0]?.uri || vscode.Uri.file('.'),
            `search-results-${results.searchTerm}-${Date.now()}.md`
        ),
        filters: {
            'Markdown': ['md'],
            'JSON': ['json'],
    ...
    try {
        ...
        const openFile = await vscode.window.showInformationMessage(
            `Search results exported to ${vscode.workspace.asRelativePath(saveUri)}`,
            'Open File'
        );
